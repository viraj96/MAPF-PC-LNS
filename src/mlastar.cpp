...
Path
MultiLabelSpaceTimeAStar::findPathSegment(ConstraintTable& constraintTable,
                                          int startTime,
                                          int stage,
                                          int lb)
{
    ...
    while (!openList_.empty()) {
        ...
        if (current->location == goalLocations[stage] && current->timestep >= holdingTime) {
        ...
        for (int successor : successors) {
            ...
            if (max(constraintTable.size, constraintTable.latestTimestep) + 1 < current->timestep) {
            ...
            if (constraintTable.constrained(successor, nextTimestep) ||
                constraintTable.constrained(current->location, successor, nextTimestep)) {
                continue;
            }
            ...
            int successorHVal = max(getHeuristic(stage, successor), holdingTime - nextTimestep);
            int successorInternalConflicts = current->numOfConflicts;
            auto* next = new MultiLabelAStarNode(current,
                                                                successor,
                                                                successorGVal,
                                                                successorHVal,
                                                                nextTimestep,
            ...
            if (next->stage == goalLocations.size() - 1 && successor == goalLocations.back() &&
                ...
                if (!(*it)->inOpenlist) {
                    ...
                    if ((successorGVal + successorHVal) <= lowerBound_) {
                        if ((*it)->getFVal() > lowerBound_) {
                            ...
                            updateInFocal = true; // Old node could be in focal list so need to update
                        }
                    }
                    if ((*it)->getFVal() > successorGVal + successorHVal) {
                        updateOpen = true; // This node would not have been popped yet from the open list
                    }
                    (*it)->copy(*next);
                    if (updateOpen) {
